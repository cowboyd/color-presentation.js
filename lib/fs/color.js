(function() {
  ...
  Color.reopenClass({
    ...
    fromHSL: function(values) {
      var rgbValue = HSLValue.create({
        h: hCoord(values.h),
        s: SLCoord(values.s),
        l: SLCoord(values.l)
      }).get('rgbValue')
      var key = JSON.stringify(rgbValue.get('rgb'))
      return cache[key] || (cache[key] = Color.create({rgbValue: rgbValue}))
    }
  })
  ...
  function hCoord(value) {
    return new Number((parseFloat(value) || 0).toFixed(2)) % 360
  }
